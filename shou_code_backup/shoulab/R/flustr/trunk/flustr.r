...
flustr <- function(file=NULL, fcs=NULL, live.colors,
                   beads=TRUE,
                   bead.gate=list(green=c(1e3,Inf),
                                  orange=c(1e5,Inf),
                                  cyan=c(1e3,Inf)),
                   cell.dilution=2.2,
                   bead.dilution=11.2, beads.per.ul=8.3e3,
                   all.bright=4, all.dim=1, 
                   fsc.sig=1, dead.sig=0, cluster.sig=1,
                   initial.cluster.min=5,
                   initial.cluster.max=15,
                   save.debug=FALSE,
                   save.unmerged=FALSE,
                   output=NULL,
                   result.path=".",
                   ...
                   max.events=NULL, ...)
{
  ...
  file.name  <- sub("(.*)\\/(.*)\\.fcs", "\\2", 
                    keyword(all)$FILENAME, perl=TRUE)
  plot.name  <- file.name
  ...
  open.plot(fsc.dead.plot,plot.type,nplots=1)
  ...
  if (warns$no.dead && warns$no.low.fsc) {
    ...
    plot(processed$tf.data, initial.chans, smooth=FALSE,
         ylim=range(dead.dat), main=file.name, pch=".", axes=FALSE)
  ...
  logicle.axis(2, tf.obj=processed$tf.objs[[dead.channel]], las=2)
  ...
  if (nrow(initial.pops$cluster.pop) > MIN_CLUSTER_POP) {
    ...
    cluster.res <- cluster.flowMeans(dat=initial.pops$cluster.pop, 
                                     ...
                                     maxn.start=30,
                                     max.attempts=10, max.streak=51,
                                     chans=live.channels,
                                     addNoise=TRUE, Standardize=TRUE, 
                                     nstart=100)
    cat("done!\n")
    if (!is.null(cluster.res)) {
      if (save.unmerged) {
        open.plot(unmerged.plot, plot.type, nplots=1)
        par(mar=c(4.7,5,3,1.5), cex=2, cex.axis=1.5, cex.lab=1.5, mex=0.6)
        cat("Plotting unmerged plot...")
      ...
      open.plot(superclust.plot, plot.type,nplots)
      ...
      if (plot.type=="pdf") {
        ...
        opar <- par(mar=c(4.2,4.2,3,1), cex=3, 
                    mfrow=c(nrows,ncols), mex=0.61)
      }

      cat("Plotting live.dead...")
  ...
  if (save.debug) {
    ...
    suppressWarnings(write.table(data.frame(res),
                                 file=debug.path,sep=sep,
                                  ...
                                  boundaries, chans, plot.name)
{
  ...
  for (lab in split(cluster.info,cluster.info$label)) {
    points(lab[lab$channel=="fsc",]$median,
           lab[lab$channel=="dead",]$median,
           pch="+",col="purple",cex=3)
  }
}

open.plot <- function(name, type, nplots) {
  if (nplots==2) h=5 else h=10
  if (type=="pdf") {
    pdf(name, width=10, height=h, useDingbats=FALSE)
  } else {
    bitmap(name, width=11.5, height=h, units="in", res=300)
  }
}
...
plot.live.dead <- function(processed, chans, pops) {
  processed.dat <- exprs(processed$tf.data)
  fsc.range  <- range(processed.dat[,chans["fsc"]])
  dead.range <- range(processed.dat[,chans["dead"]])
  plot(processed$tf.data, chans, type="n", smooth=FALSE,
       xlim=fsc.range, ylim=dead.range, axes=FALSE, ann=FALSE)
  mtext(side=1,line=2.7,text=chans["fsc"])
  mtext(side=2,line=2.7,text=chans["dead"])
  ...
  axis(1,at=axTicks(1),labels=parse(text=paste("10^",axTicks(1))))
  logicle.axis(2, tf.obj=processed$tf.objs[[chans[2]]], las=2)
  box()

  par(new=FALSE)
}

plot.fmclusters <- function(dat, clust.res, clust.dat, tf.objs, chans) {
  cols <-list(
              'red'    = "red",
              'green'  = "green",
              'orange' = "orange",
  ...
  for (i in 1:(length(chans)-1)) {
    for (j in (i+1):length(chans)) {
      ...
      plot(dat,c(chans[i],chans[j]),type="n",
           cex=3, smooth=FALSE, axes=FALSE,ann=FALSE)
      mtext(side=1,line=2.7,text=chans[i])
      mtext(side=2,line=2.7,text=chans[j])
      logicle.axis(1, tf.obj=tf.objs[[chans[i]]])
      logicle.axis(2, tf.obj=tf.objs[[chans[j]]], las=2)
      box()

      par(new=TRUE)
      mbids <- matrix(nrow=sum(grepl("moribund",sizes$name)),
                      ncol=3,
                      dimnames=list(NULL,c("id","x.med","y.med")))
      ...
      for (chan in sizes$name) {
        ...
        if (grepl("moribund",nme)) {
          ...
          mbids[idx,] <- cbind(id,x.med,y.med)
          nme <- "moribund"
          idx <- idx+1
        }
...
#print(dat)
  if (save.debug) {
    suppressWarnings(write.table(dat, file=debug.path, sep=sep, 
                                 row.names=FALSE, append=TRUE))
  }
  dat
}
...
order.clusters <- function(dat, cluster.res, chans) {
  ...
  nrows <- nlabs*nchans
  exprs.dat <- exprs(dat)
  res <- matrix(nrow=nrows,ncol=4)
  chan.vec <- vector("character",length=nrows)
  idx <- 1
  for (lab in 1:nlabs) {
    for (cn in 1:nchans) {
      chan <- chans[cn]
      ...
      res[idx,] <- cbind(lab, 
                         ...
                         mad(cluster.dat), 
                         length(cluster.dat))
      chan.vec[idx] <- names(chan)
      idx <- idx+1
    }
  }
  res <- data.frame(res,chan.vec)
  names(res) <- c("label","median","mad","n","channel")
  res <- data.frame(res[order(res$channel,-res$median),],
                    ranks=rep(1:(nrows/nchans),times=nchans))
}
...
make.result <- function(file.name, cells, low.fsc, beads, dead, fluor,
                        cell.dilution, bead.dilution, beads.per.ml)
{
  res <- NULL
  categories <- list()
  ...
  res$density <- data.frame(file=file.name,
                            cell.dilution*(res$count[2:ncol(res$count)]/
...
transform.data <- function(dat, chans, save.debug, debug.path, sep) {
  ...
  nfluor.chans <- sum(!names(chans) %in% c("fsc","ssc"))
  transf     <- list()
  all.params <- matrix(nrow=nfluor.chans,ncol=5)
  chan.vec   <- vector("character",length=nfluor.chans)

  idx <- 1
  for (cn in chans) {
    if (cn %in% chans[c("fsc","ssc")]) {
      transf[[cn]] <- shiftLogTransform(cn)
    } else {
      params <- logicle.params(dat[,cn])
      all.params[idx,] <- unlist(params)
      chan.vec[idx]    <- cn
      transf[[cn]] <- logicleTransform(cn, t=params$t, w=params$w, 
                                       m=params$m)
      idx <- idx+1
    }
  }
  all.params <- data.frame(chan.vec, all.params)
  names(all.params) <- c("channel","lt.zero","w","t","m","p")
  print(all.params)

  if (save.debug) {
    suppressWarnings(write.table(all.params, file=debug.path, sep=sep, 
                                 row.names=FALSE, append=TRUE))
  }
  ...
  res <- list(tf.data=transDat, tf.params=all.params, tf.objs=transf)
                              ...
                              maxn.start, chans, ...)
{
  ...
  len.maxn.start <- length(maxn.start)
  maxn <- if(len.maxn.start > 1) {
            if (max.attempts > len.maxn.start) { 
              max.attempts <- len.maxn.start
            }
            maxn.start
          } else {

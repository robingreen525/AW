...
#   source("/var/www/projects/flustrw/bin/flustr.r")
    source("/home/nodice/Documents/Code/R/flustr/flustr.r")
  ...
  for (folder in 1:length(data.location)) {
    ...
    not.fluor <- c("file","beads","dead","live","low.fsc")
    totals    <- not.fluor[3:5]
    ...
    live.ratios  <- 
      final.counts[!names(final.counts) %in% not.fluor]/
      final.counts[["live"]]

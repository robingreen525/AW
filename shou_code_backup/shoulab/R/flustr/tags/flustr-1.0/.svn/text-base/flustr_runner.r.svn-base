                          ...
                          ...)
{
  ...
  require(flowClust)
  require(flowViz)
  source("~/Documents/Code/R/flowcore/flustr.r")

  fcs.files <- 
    list.files(path=data.location,pattern="*.fcs",full.names=TRUE)
  nFiles <- length(fcs.files)

  result.folder <- 
    paste(format(Sys.time(),"%Y%d%m_%H%M%S"),"-flustr_results",sep="")

  result.path <- paste(result.location,"/",result.folder,sep="")

...
#  contour(processed, c(fsc,dead), add=TRUE)
  ...
  first.cut <- cluster.flowMeans(dat=processed, min.clusters=4,
                                 max.clusters=11, maxn.start=20,
  ...
  cluster.res <- cluster.flowMeans(dat=cluster.pop, min.clusters=15,
                                   max.clusters=25, maxn.start=50,
...
#                                   Update="Means",
  ...
  cluster.data <- order.clusters(dat=cluster.pop, 
                                 ...
                                 chans=live.channels)
  print(cluster.data)
  cluster.data <- name.fmclusters(dat=cluster.data, significant=1,
  ...
  if (nrow(dead.pop) > 0) {
    ...
    contour(dead.pop, c(fsc,dead), add=TRUE, col="green")
    par(new=TRUE)
  }
  if (nrow(low.fsc.pop) > 0) {
    plot(low.fsc.pop, c(fsc,dead) ,pch=".", cex=2, smooth=FALSE, 
         xlim=c(min.fsc,max.fsc), ylim=c(min.dead,max.dead),
        col="brown", ann=FALSE, axes=FALSE)
...
plot.fmclusters <- function(dat, clust.res, clust.dat) {
  ...
  pchs <-list(`FITC-A`='.', `PE-A`='.', `AmCyan-A`='.',`APC-A`='.',
              ...
              moribund=".")
  chans <- as.character(unique(clust.dat$channel))
#  par(new=FALSE)
#  x11()
...
cluster.flowMeans <- function(data, min.clusters, max.clusters,
                              ...
                              maxn.start, chans, ...)
{
  ...
  while (nlabs < min.clusters || nlabs > max.clusters) {
    ...
    plot(data, tries[[attempt]], c(chans), pch=".", cex=3)

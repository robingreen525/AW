logicle.axis <- function(side, params, type="logicle", ...) {
  ...
  tics <- logicle.tics(params, limits)
...
logicle.tics <- function(params, lims) {
  vals     <- invLogicle(params, lims)
  ...
  logicle.maj <- dec.to.logicle(maj.tics$values, params) 
  logicle.min <- dec.to.logicle(min.tics, params)
...
invLogicle <- function(params, ticks) {
  lead.term <- params$t * 10^(params$w-params$m)

  x.gt.w <- ticks[ticks>params$w]
  x.lt.w <- ticks[ticks<params$w]

  a <- lead.term * (10^(params$w-x.lt.w) - params$p^2 * 
       10^(-((params$w-x.lt.w)/params$p)) + params$p^2 - 1)
  b <- lead.term * (10^(x.gt.w-params$w) - params$p^2 * 
       10^(-((x.gt.w-params$w)/params$p)) + params$p^2 - 1)

plater <- function( initial.vol=NULL, ex.v, ex.vf, ex.d, plating.vol, runs, all.data=FALSE, verbose=FALSE )
{
  ...
  tot <- runs * length(ex.d)

  obs.densities <- array( 0, dim=c(runs, n.samples, length(ex.d)) )
  ex.densities  <- matrix( 0, nrow=length(ex.d), ncol=n.samples )
  ex.densities <- t(apply(as.array(ex.d), 1, function(x) cumprod(c(x, ex.v/ex.vf, 1))))


  error <- array(0, dim=c(runs, n.samples, length(ex.d)))
  plated <- matrix(0, nrow=n.samples, ncol=length(ex.d))

  ex.d <- rep(ex.d, each=runs)
  ...
  if (!is.null(initial.vol.ml)) {
    ...
    v <- take.volume(initial.vol.ml, p, tot, ex.d)
  ...
  for (i in 1:n.dils) {
    ...
    v <- take.volume(ex.v.ml[i], p, tot, samp)
    V <- take.volume(ex.vf.ml[i]-ex.v.ml[i], P, tot)
    VT <- v[,"a.vol"]+V[,"a.vol"]
  ...
  samp <- obs.densities[,step-1,]
  v    <- take.volume(plating.vol.ml, p, tot, samp)

  obs.densities[,step,] <- v[,"a.cells"]/plating.vol.ml
  #print(obs.densities)
  #print(ex.densities)
  #for (i in 1:dim(obs.densities)[length(dim(obs.densities))]) {
  #  error[,,i] <- apply(obs.densities[,,i], 1, 
  #                function(x) abs(ex.densities[i,] - x) / ex.densities[i,])
  #}
  plated <- obs.densities[,ncol(obs.densities),]*plating.vol.ml
  ...
  if (identical(all.data, TRUE)) {
    ...
    #return( ( plated, error[,ncol(error),] ) )
    plated
  }
}
                 ...
                 runs, step.size=NULL, search.range=NULL )
{
  ...
  #probs <- NULL
  idx <- 1
  res <- matrix(0, nrow=runs, ncol=total.steps)
  res <- plater( ex.d=seq(min.d,max.d,step), initial.vol=initial.volume,
                 ex.v=sample.volumes, ex.vf=dilution.volumes,

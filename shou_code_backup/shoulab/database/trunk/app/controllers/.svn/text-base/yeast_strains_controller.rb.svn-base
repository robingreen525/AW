class YeastStrainsController < ApplicationController
  ...
  def new
    ...
    @yeast_strain.number = get_new_number()
  ...
  def copy
    old = YeastStrain.find(params[:id])
    @yeast_strain = YeastStrain.new(old.attributes)
    @yeast_strain.number = get_new_number()
  end
...
private
  def get_new_number
      return YeastStrain.find(:first, :order => "number DESC").number + 1
  end

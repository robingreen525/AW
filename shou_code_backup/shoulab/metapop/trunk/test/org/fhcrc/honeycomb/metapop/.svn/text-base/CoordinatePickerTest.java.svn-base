...
public class CoordinatePickerTest {
    ...
    long seed = 12345L;
    RandomNumberUser rng = new RandomNumberUser(seed);

    CoordinateProvider prov;

    @Before
    public void setUp() {
        prov = new Provider(2, 2, new Coordinate(1,2));
    }
    ...
    public void specified() {
       ...
       CoordinatePicker spec =
           new SpecifiedCoordinatePicker(initial_locations);
    ...
    public void specifiedError() {
       CoordinatePicker spec =
           new SpecifiedCoordinatePicker(null);
       spec.pick();
    }
    ...
    public void specifiedError2() {
       CoordinatePicker spec =
           new SpecifiedCoordinatePicker(Arrays.asList(new Coordinate(1,1)));
       spec.pick(2);
    }

    @Test(expected = NullPointerException.class)
    public void currentCoordNotSet() {
        CoordinateProvider prov = new Provider(2,2);
        boolean exclude_current = true;

        CoordinatePicker random_picker = new RandomPicker(exclude, prov, rng);
        random_picker.pick();
    }

    @Test(expected = IllegalStateException.class)
    public void limitsNotSet() {
        CoordinatePicker rand = new RandomPicker(rng);
        rand.pick();
    }

    @Test(expected = IllegalArgumentException.class)
    public void cantBeUnique() {
        CoordinatePicker uni = new UniqueRandomPicker(include, prov, rng);
        uni.pick(10);
    }
    ...
    public void randomPickerExclude() {
        ...
        random_picker = new RandomPicker(include, prov, rng);
        boolean same = false;
        for (int i=0; i<100; i++) {
    ...
    public void checkSizes() {
        int max_distance = 1;
        int samples = 10;
        prov = new Provider(10, 10, new Coordinate(3, 3));

        CoordinatePicker rand  = new RandomPicker(include, prov, rng);
        CoordinatePicker uni   = new UniqueRandomPicker(include, prov, rng);
        CoordinatePicker neigh = new RandomNeighborPicker(exclude, prov,
                                                          max_distance, rng);
    ...
    public void testNeighborhood() {
        ...
        neigh = new RandomNeighborPicker(exclude, prov, max_distance, rng);

                            ...
                            ylim=c(1e-2,1e6))
{
    ...
    for (i in 2:(n.plot.files)) {
        ...
        do.plot <- function(pair) {
            ...
            if (plot.type=="cell") {
                if (show.resource) {
                    if (length(timepoint.ss) != length(resource)) {
                        points(timepoint.ss[2], resource, pch=6, lwd=linew)
                    } else if (resource[1] < 1e-4) {
                        points(timepoint.ss[2], resource[2], pch=6, lwd=linew)
                    } else if (resource[2] < 1e-4) {
                        if (first) {
                            points(timepoint.ss[1], resource[1],
                                   pch=4, lwd=linew)
                            first <- FALSE
                        }
                    } else {
                                   ...
                                   name="growth_rate_vs_release", ...)
{
    ...
    abline(fit, col=gray(0.4), lty=2, lwd=5)
    box()
                               ...
                               name="survival_prob_vs_release", ...)
{
    ...
    fit <- nls(survival.prob~(p*(release.rate-x))/ (k+(release.rate-x)),
               data=dat, start=list(p=p,k=k,x=x), weights=1/extinct)

    p.max <- coef(fit)['p']
    k.survive <- coef(fit)['k']
    x.shift <- coef(fit)['x']

    curve((p.max*(x-x.shift))/(k.survive+(x-x.shift)), add=TRUE,
           col=gray(0.5), lty=2, lwd=5, from=x.shift)
    ...
    for (condition in conditions) {
        ...
        for (j in seq_along(runs)) {
            ...
            if (length(coop.freqs) != 1 && 
                all.hrs[length(all.hrs)] < end.tolerance)
            {
                cat("\trun", run.id,
    ...
    for (mig.range in split(dat, dat$range)) {
        ...
        for (s2 in split(mig.range, mig.range[[split2]])) {
            ...
            for (i in seq_along(by.last)) {
                m <- 1
                est <- c()
                for (mr in c(split(by.last[[i]], by.last[[i]]$mig))) {
                    last.s <- unique(mr[last.split])
                    mig.rate <- unique(mr$mig)
                    survived <-

            ...
            if (plot.type=="cell") {
                ...
                for (type in data.cols) {
                    ...
                    if (length(timepoint.ss) != length(pair[[type]])) {
                        points(timepoint.ss[2], pair[[type]], pch=6,
                         ...
                         ts.scale=100) 
{
    ...
    for (dat.file in list.files(folder, full.names=TRUE)) {
        ...
        if (nrow(analysis.range)>0) {
            # find the locations that started growing earliest.
            earliest.row.cols <-
                analysis.range[analysis.range$hrs==min(analysis.range$hrs),]$row.col
            earliest <-
                analysis.range[analysis.range$row.col %in% earliest.row.cols,]

            resource.mean <- mean(earliest$resource.before)
            resource.per.cell <- mean(earliest$resource.before/earliest$coops)

            for (r.try in 100:1) {
                r <- max.r/r.try
                fit <- try(nls(coops ~ A*exp(r*hrs), data=earliest,
                               start=list(A=A,r=r)),
                           silent=TRUE)
        ...
        res <- rbind(res, data.frame(release.rate=release.rate,
                                     extinct=extinct,
                                     total=size,
                                     resource.mean = resource.mean,
                                     resource.per.cell=resource.per.cell,
...
plot.extinction <- function(dat, device="x11", save.to=".", cutoff=0.05, ...) {
    graphics.off()
    library(binom)
    ...
    for (mig.range in split(dat, dat$range)) {
        for (mig.range.occ in split(mig.range, mig.range$occ)) {
            ...
            cond <- unique(names(by.last))
            if (last.split=="mutant-freq") { 
                cond <- as.numeric(cond)*unique(mig.range.occ$n)
            }
            for (i in seq_along(by.last)) {
                m <- 1
                est <- c()

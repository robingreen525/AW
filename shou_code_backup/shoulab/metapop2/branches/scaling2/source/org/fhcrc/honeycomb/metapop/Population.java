 ...
 * @author Adam Waite
 * @version $Id: Population.java 2241 2013-08-01 22:02:26Z ajwaite $
 *
 */
public class Population {
    private static final double RESOURCE_TOL = -1e-13;
     ...
     * 
     * 2. For each subpopulation:
     *      2.1 The current cells release resource, as appropriate.
     *      2.2 If the amount of requested resource is greater than the 
     *          amount of available resource, the number of births is scaled.
     *      2.3 The integer part of the scaled births is added to the
     *          subpopulation, while the remainder is added to that
     *          subpopulation's "unfinished cells".  The subpopulation handles
     *          when this fraction becomes a new cell.
     *
    ...
    public void grow() {
        ...
        for (Subpopulation subpop:subpopulations) {
            ...
            // births only occur below carrying capacity.
        ...
        for (int i=0; i<n_subpopulations; i++) {
            ...
            int subpop_size = subpop.getSize();

            int deaths = subpop.getDeaths(available_resource);

            int    unscaled_births = birth_list.get(i);
            double scaled_births = unscaled_births;
            int    scaled_births_floor = unscaled_births;

            if (overconsumed) {
                double scale_factor = available_resource/requested_consumption;
                scaled_births = unscaled_births * scale_factor;
                scaled_births_floor = (int) StrictMath.floor(scaled_births);

                // This may increase the size of the subpopulation.
                subpop.addToUnfinishedCells(scaled_births-scaled_births_floor);
            }

            // Consumption occurs according to scaled births.
            double consumed = subpop.getGamma()*(scaled_births);
            double released = subpop.getReleaseRate()*subpop_size; 


            total_released += released;
            total_consumed += consumed;

            // Size changes according to floor of scaled births.
            subpop.setSize(subpop.getSize() + scaled_births_floor - deaths);
            resource += released - consumed;
        }

        if (resource < RESOURCE_TOL)
            throw new RuntimeException("resource < 0");

 ...
 * @author Adam Waite
 * @version $Id: PopulationTest.java 2183 2013-06-27 23:30:43Z ajwaite $
 *
 */
public class PopulationTest {
    ...
    private double saturation_death_rate = 0.7;
    ...
    @Test
    public void sizeAfterGrowth() {
        int expected_size = initial_total_size * 2;
        doubling_pop.grow();
        assertEquals("wrong size after growth", 
                     expected_size,
                     doubling_pop.getSize());
    }

    @Test
    public void saturatedSizeAfterGrowthNoDeath() {
        doubling_pop.setCapacity(capacity);
        doubling_pop.grow();
        assertEquals("growth after saturation",
                     initial_total_size, doubling_pop.getSize());
    }

    @Test
    public void saturatedSizeAfterGrowthWithDeath() {
        doubling_death_pop.setCapacity(capacity);
        //System.out.println("before: " + doubling_death_pop);
        doubling_death_pop.grow();
        //System.out.println("after: " + doubling_death_pop);
        assertTrue("no death during saturation",
                   initial_total_size > doubling_death_pop.getSize());
    }

    @Test
    public void limitedNutrientGrowth() {
        int size = limited_half_pop.getSize();
        double resource = limited_half_pop.getResource();

        limited_half_pop.grow();

        int new_size = limited_half_pop.getSize();
        double new_resource = limited_half_pop.getResource();

        assertEquals("biomass conversion wrong",
                     new_size - size, resource - new_resource, 0.0);
    }

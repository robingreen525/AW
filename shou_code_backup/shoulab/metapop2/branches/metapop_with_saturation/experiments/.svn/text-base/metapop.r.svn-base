...
plot.folder <- function(folder, save.path, run.pattern, device="svg", ...)
{
                            ...
                            show.resource=FALSE, rsample, xlim, ylim)
{
    ...
    for (i in 2:(n.plot.files)) {
        ...
        if (i==2) {
            rows <- if (is.null(eval(match.call()$rsample)) ||
                        identical(row.col, "all"))
            {
                1:nrow(prev.all$dat)
            } else {
                sample.int(n=nrow(prev.all$dat), size=rsample)
            }
            ...
            if (plot.type=="cell") {
                ...
                for (type in data.cols) {
                    ...
                    if (length(timepoint.ss) != length(pair[[type]])) {
                        points(timepoint.ss[2], pair[[type]], pch=6, lwd=linew,
                               col=color[[type.name]])
                    } else {
        ...
        if (identical(row.col, "total")) {
            ...
            if (identical(row.col, "all")) {
                ...
                show.resource <- FALSE
                paired <- rbind(prev.all$dat[rows,], current.all$dat[rows,])
            } else {
...
release.test <- function(folder) {
    ...
    for (dat.file in list.files(folder, full.names=TRUE)) {
        ...
        dat <- within(dat, hrs <- timestep/100)

        extinct <- nrow(subset(dat, dat$timestep==max(dat$timestep) & 
                               dat$anc_coop==0))
        ...
        if (release.rate < 1) {
            ...
            if (release.rate >=1 && release.rate <1.5) {
                r <- r/7
                hr.range <- c(90, 100)
            } else if (release.rate >= 1.5 && release.rate < 1.9) {
                r <- r/5
                hr.range <- c(58,65)
            } else if (release.rate >= 1.9 && release.rate < 2.5) {
                r <- r/2
                hr.range <- c(38,45)
            } else if (release.rate >= 2.5 && release.rate < 100) {
                r <- r/2
                hr.range <- c(33,40)
            } else if (release.rate >= 100) {
                r <- r/2
                hr.range <- c(25,35)
            }

            fit.ss <- subset(dat, hrs >= hr.range[1] & hrs <= hr.range[2] &
                             anc_coop > 0)
            resource.ss <- subset(fit.ss, anc_coop>1000)
            resource.mean <- mean(resource.ss$resource)
            resource.per.cell.mean <- mean(resource.ss$resource/resource.ss$anc_coop)
            #if (release.rate==2.5) browser()
            #fit.ss <- subset(fit.ss, 
            #                 anc_coop > quantile(fit.ss$anc_coop, probs=0.1))

            #plot(resource/anc_coop~I(timestep/100), data=fit.ss, type="n")
            #for (s in split(fit.ss, fit.ss$row.col)) {
            #    points(resource/anc_coop~I(timestep/100), data=s, type="o")
            #}

            fit <- nls(anc_coop ~ A*exp(r*hrs), data=fit.ss,
                       start=list(A=A,r=r))
            ci <- confint(fit)['r',]
            ...
            res <- rbind(res, data.frame(release.rate=release.rate,
                                         extinct=extinct,
                                         total=size,
                                         resource=resource.mean,
                                         resource.per.cell=resource.per.cell.mean,
                                         growth.rate=coef(fit)['r'],
                                         lower=ci[1], upper=ci[2]))
        }
    }
    res[order(res$release.rate),]
    res <- within(res, survival.prob <- 1-(extinct/total))
    res
}

plot.growth.vs.release <- function(dat,device="pdf",
                                   name="growth_rate_vs_release", ...)
{
    graphics.off()
    mtext.cex <- 2.2
    default.plot(10,10,device=device,name=name)
    plot(growth.rate~release.rate, data=dat, ann=FALSE, axes=FALSE, ...)
    arrows(dat$release.rate, dat$lower, dat$release.rate, dat$upper, code=3,
           angle=90, length=0.1)
    axis(1,lwd=par()$lwd, cex.axis=1.2)
    axis(2,lwd=par()$lwd, las=2, cex.axis=1.2)
    mtext(side=1, text="release rate (units/hr)", line=3.0, cex=mtext.cex)
    mtext(side=2, text="growth rate (/hr)", line=4.5, cex=mtext.cex)

    fit <- lm(growth.rate~release.rate, data=dat, subset=release.rate >=1 &
              release.rate < 2.5)
...
plot.summary <- function(dat, device="x11", save.to, jit=NULL, gap=2, ...)
{
    ...
    for (type in split(dat, dat$type)) {
        for (type.occ in split(type, type$occ)) {
            plot.name <- paste(unique(type.occ$type), ", ",
                               "occ=", unique(type.occ$occ), sep="")
            ...
            color <- color.start
            by.last <- split(type.occ, type.occ[[last.split]])
            ...
            for (i in seq_along(by.last)) {
                ...
                mut <- within(mut, {lower <- ifelse(coop.freq.sd>1e-3,
                                    ...
                                    upper <- ifelse(coop.freq.sd>1e-3,
                                                    y.jit+coop.freq.sd, NA)})
                points(x.jit, y.jit, col=color, pch=color, cex=1.0)
...
generate.run <- function(conds) {
    browser()
}

plot.extinction <- function(dat, device="x11", save.to, cutoff=0.1, ...) {
    graphics.off()

    y.pad <- 0.1

    dat <- dat[order(dat$mig),]
    x.label <- c(0, parse(text=paste("10^",
                                     log10(unique(dat[dat$mig>0,]$mig)))))
    n.migs <- length(unique(dat$mig))
    x.at <- 1:n.migs
    x.range <- range(x.at)
    y.range <- c(0-y.pad, 1+y.pad)
    ...
    for (type in split(dat, dat$type)) {
        for (type.occ in split(type, type$occ)) {
            plot.name <- paste(unique(type.occ$type), ", ",
                               "occ=", unique(type.occ$occ), "_extinct_freq",
                               sep="")
            plot.path <- file.path(save.to, plot.name)
            if (!file.exists(save.to)) dir.create(save.to, recursive=TRUE)
            ...
            for (i in seq_along(by.last)) {
                ...
                for (mr in split(by.last[[i]], by.last[[i]]$mig)) {
                    ...
                    bnm <- binom.wilson(x=extinct, n=tot)
                    code <- if (bnm$mean==0) {
                        2
                    } else if (bnm$mean==1) {
                        1
            ...
            axis(2, las=2, lwd.ticks=par()$lwd)
            box()
            mtext(side=1, text="Migration rate (/hr)", line=3.2, cex=2.5)
            mtext(side=2, text="Coop. extinction frequency", line=5, cex=2.5)
            legend(x=1, y=0.8,
                   legend=cond,
                   title=leg.title,
                   col=color.start:(color-1), pch=color.start:(color-1),

...
class AdaptiveRaceParams(object):
    ...
    def set_env(self, hostname):
        ...
        if hostname == 'Nietzsche':
            ...
            self.java = "time java -Xmx4000m -server -Djava.io.tmpdir=$TMPDIR"
            self.sbatch = "sbatch --mem=4000 -n1 -t{} --wrap='".format(self.time)
            self.end = "'"
    ...
    def build_commands(self):
        ...
        args = list(list(i) for i in product(
                                             ...
                                             params['evo_adv'],          #9
                                             params['evo_trade'],        #10
                                             params['initial_resource'], #11
                                             params['size'],             #12
                                             params['frac_occupied'],    #13
                                             params['migration_rates'],  #14
                                             params['coop_to_cheat'],    #15
                                             params['cheat_to_coop']))   #16

        output = self.output
        if output == '': output = self.__class__.__name__
        outputs = []
        cmds = []
        template = '{0}_{1}_n={2}_mutant-freq={3}_coop-release={4}_' \
                   'coop-freq={6}_size={12}_occ={13}_mig={14}_' \
                   'coop-to-cheat={15}'
...
class ReleaseTest(AdaptiveRaceParams):
    def __init__(self):
        ...
        self.mutant_freqs = [0]
        self.initial_pop_size = [1]
        self.coop_release = [x + y/10.0 for x in range(1) for y in range(10)]

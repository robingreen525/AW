...
flustr <- function(file=NULL, fcs=NULL, live.colors,
                   ...
                   fsc.sig=1, dead.sig=0, cluster.sig=0.5,
...
plot.live.dead <- function(processed, chans, pops) {
  ...
  plot(processed$tf.data, chans, type="n", smooth=FALSE,
       xlim=fsc.range, ylim=dead.range, axes=FALSE, cex.lab=0.8)
  ...
  for (i in 1:(length(chans)-1)) {
    for (j in (i+1):length(chans)) {
      ...
      plot(dat,c(chans[i],chans[j]),type="n",
           cex=3, smooth=FALSE, axes=FALSE,cex.lab=0.8)
...
find.minima <- function(dat,chans) {
  ...
  fsc.gate <- matrix(c(log10(1), log10(5.4e5)),

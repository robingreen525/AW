                   ...
                   remote=TRUE, ...)
{
  ...
  plot.base       <- paste(data.path,"/",plot.name,sep="")
  superclust.plot <- paste(plot.base,".pdf",sep="")
  unmerged.plot   <- paste(plot.base,"_unmerged.pdf",sep="")
  fsc.dead.plot   <- paste(plot.base,"_fsc-dead.pdf",sep="")
  ...
  first.cut <- cluster.flowMeans(dat=processed, 
                                 ...
                                 nstart=100, plot.name=fsc.dead.plot)

  nlabs <- length(unique(first.cut@Label))

  pdf(fsc.dead.plot,width=10,height=10,useDingbats=FALSE)
  par(mar=c(4.7,5,3,1.5), cex=2, cex.axis=1.5, cex.lab=1.5, mex=0.6)
  plot(processed,first.cut, c(fsc,dead), main=file.name, pch=".")

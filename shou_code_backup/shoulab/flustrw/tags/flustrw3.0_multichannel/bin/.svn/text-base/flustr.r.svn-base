...
check.channels <- function(arg.chans, data.chans) {
  ...
  seen <- ""
  for (cn in arg.chans) {
    if (any(cn %in% seen)) stop ("FLUSTR ERROR: Duplicate channels!")
    seen <- c(seen,cn)
  }
}
                           ...
                           cluster.pop, dead.pop, low.fsc.pop)
{
  ...
  if (nrow(cluster.pop)>0) {
    ...
    contour(cluster.pop, chans, add=TRUE)
    par(new=TRUE)
  }
  if (nrow(dead.pop) > 0) {
    ...
    contour(dead.pop, chans, col="orange", add=TRUE)
    par(new=TRUE)
  }
  if (nrow(low.fsc.pop) > 0) {
    ...
    contour(low.fsc.pop, chans, add=TRUE)
  }
  par(new=FALSE)
}

...
#        dat <- NULL
        ...
        for (current.chrom in chrom.order) {
            ...
            in.both.data <- 
                na.omit(subset(dat, chrom==current.chrom &
                               !(log2.chrom.ratio %in% c(-999,999))))
            not.in.evo.pos    <- subset(dat,log2.chrom.ratio==-999)$pos
            not.in.anc.pos    <- subset(dat,log2.chrom.ratio==999)$pos
            not.in.either.pos <- subset(dat,is.na(log2.chrom.ratio))$pos

            chrom.windowed <- sliding.window(in.both.data,smooth.window)
            ...
            cnv.plot(ratio.data=chrom.reduced, x.ticks=chrom.x.ticks, sds,
...
plot.depthstats <- function(file.location,chr.order) {
    ...
    chroms <- gsub("chr","",depth.dat$chromosome)
    ...
    plot(depth.dat$chromosome, rep(y.range,length(xs)/2), type="n", 
         ylab="mean coverage", xlab="", main=name, border="white",
         axes=FALSE, ann=FALSE)
    axis(1,at=1:length(chroms),labels=chroms, lwd.ticks=par()$lwd,
         cex.axis=0.9)
    axis(2,lwd.ticks=par()$lwd)
...
coverage.plot <- function(cov.data, column, x.ticks, strain) {
    ...
    lines(cov.data$pos,coverage)
    points(cov.data$pos,coverage, col=ifelse(coverage<5,"magenta","black"))
}

cnv.plot <- function(ratio.data, x.ticks, sds, not.in.evo,
                     not.in.anc, not.in.either)
{
...
test.plot.cnv <- function() {
    ...
    strain.paths <- 
        paste(pfx,c("cc/Sample_JR30"),sep="/")
